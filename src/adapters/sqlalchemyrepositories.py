...
from infraestrucutre.models import UserTable
...
class SqlAchemyRepositories(RepositoriesInterface):
    ...
    def create_user(self, user:User):
        user = UserTable(name=user.name, email=user.email, state=user.state, city=user.city, user_type=1)
        self.db.session.add()
        self.db.session.commit()
        return user

    def get_user_by_email(self, email:str):
        user = self.db.session.query(UserTable).filter_by(email=email).first()
        if user:
            return User(user.name, user.email, user.state, user.city)
        return None
    
    def get_users_by_city(self, city:str):
        users = self.db.session.query(UserTable).filter_by(city=city).all()
        return [User(user.name, user.email, user.state, user.city) for user in users]
    
    def get_users_by_state(self, state:str):
        users = self.db.session.query(UserTable).filter_by(state=state).all()
        return [User(user.name, user.email, user.state, user.city) for user in users]
